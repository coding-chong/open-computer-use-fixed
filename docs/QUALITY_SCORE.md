...
| Windows runtime | B | 独立 Go `.exe` 通过 Windows UI Automation、strict native HWND messages 和显式授权的 DPI-aware `SendInput` 暴露同样 10 个 tools、MCP server 和 `call --calls`；每个 action 绑定 snapshot PID/start/HWND，元素索引要求从 snapshot HWND 解析非空、类型有效且处于 signed 32-bit UIA component range 的 exact runtime ID，坐标 bounds 有一像素 freshness guard，shared executable aliases fail closed，且 PowerShell 7 runner 已通过双 WPF/WinForms safety matrix。 | 补完整 action matrix 与真实第三方 app smoke、installer/signing，以及更原生的 Go UIA 实现或更稳定的 bridge。 |
