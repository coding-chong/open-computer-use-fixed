...
- `open-computer-use call <tool> --args '{...}'` 会直接输出 MCP-style JSON result；`open-computer-use call --calls '[...]'` / `--calls-file <path>` 会在同一进程里顺序执行 JSON 数组里的 tool calls，并复用同一个 `ComputerUseService` 内存态。Windows 的 `element_index` 是最新 `get_app_state` 返回的 generation-bound opaque identifier；每次成功发布新 snapshot 后旧 identifier 失效，不能把旧的裸数字索引复用到新 snapshot。序列执行默认会在成功的相邻操作之间 sleep 1 秒，也可以用 `--sleep <seconds>` 覆盖；遇到 `isError=true` 的 tool result 后停止。
...
- Go runtime 通过 `go:embed` 带上 `runtime.ps1`，执行 tool call 时临时落盘并调用 Windows PowerShell。PowerShell bridge 使用 `System.Windows.Automation` 做 app/window/element discovery、tree rendering、UIA pattern action、ValuePattern set value 和 ScrollPattern scroll；对必须接收真实桌面输入的已授权路径，bridge 使用 `SendInput`，并在初始化时切换到 per-monitor-v2 DPI awareness，确保 UIA、窗口 bounds 和虚拟桌面绝对坐标位于同一物理坐标空间。
- Windows runtime 默认只连接已经运行的 app；找不到进程时永不从模型提供的 selector 启动可执行文件、URL 或 shell 文档。`SetFocus` secondary action 仍需要 `OPEN_COMPUTER_USE_WINDOWS_ALLOW_FOCUS_ACTIONS=1` 显式打开。`get_app_state` 默认只返回 UI Automation 状态树；需要视觉像素时传入 `include_image: true`。显式请求图片时，Windows bridge 对当前拥有前台窗口的目标先重新确认 foreground 后使用屏幕拷贝；后台目标只尝试 `PrintWindow`，检测黑图后不再用未证明归属的桌面矩形替代，而是省略图片；PNG Alpha 仍归一化为不透明；action tools 的刷新结果仍自动包含截图（若能证明捕获归属）。`type_text` 只对动作时刻当前聚焦、且通过目标进程与 snapshot-bound window 归属校验的可写文本控件生效；调用方必须先 click/select 字段，不能依赖 `focusedSummary`、树顺序或同 metadata 控件，`set_value` + `element_index`（其中传 generation-bound identifier）是精确元素赋值替代。运行时不会扫描当前 UIA 树寻找替代控件，也不会向顶层 HWND 发送兜底键盘消息；通过校验的目标优先使用 child HWND 的 `EM_SETSEL` / `EM_REPLACESEL`（以当前值的 UTF-16 长度选择文本末尾并验证追加结果），没有可用 native handle 时才在 `OPEN_COMPUTER_USE_WINDOWS_ALLOW_UIA_TEXT_FALLBACK=1` 显式授权下使用同一 focused element 的 UIA `ValuePattern.SetValue`。Windows `scroll.pages` 上限为 100；小数输入在 semantic ScrollPattern 中按当前 viewport 百分比执行一次相对滚动，在 app-scoped message fallback 中按一次对应 wheel delta 投递；超时错误要求先重新 snapshot，不得盲目重放。
- Windows top-level target resolution is proof-based: it first accepts the process `MainWindowHandle` only when that HWND is live, process-owned, top-level, on the virtual desktop, and exactly matches a positive UIA root rectangle within one physical pixel. If that proven main candidate is unavailable, it accepts exactly one independently discovered UIA desktop top-level candidate satisfying the same checks; zero or multiple candidates yield `No usable top-level interactive window is available for the requested app.` Snapshot publication, action refresh, coordinate validation, and capture re-run this same resolver, so an ownership-only, hidden, or off-screen proxy HWND cannot be revived downstream.
- `click_method=global`、`drag` 和 `press_key` 的真实系统输入路径默认关闭。全局 click 与 physical drag 要求同时设置 `OPEN_COMPUTER_USE_WINDOWS_ALLOW_FOREGROUND_INPUT=1` 和 `OPEN_COMPUTER_USE_ALLOW_GLOBAL_POINTER_FALLBACKS=1`；键盘操作要求前一项并要求目标已经拥有 foreground window，只有额外设置既有的 `OPEN_COMPUTER_USE_WINDOWS_ALLOW_FOCUS_ACTIONS=1` 才会允许运行时做有界的 `SetForegroundWindow` 尝试。physical pointer 注入会先验证请求屏幕点处的顶层窗口属于目标进程和整条 drag 路径，键盘注入会验证目标进程拥有 foreground window，并在 Windows 前台策略或 UIPI 阻断时 fail closed。未授权的 drag 仅保留 app-scoped `PostMessage` best-effort 路径：它验证 HWND 所有权、检查每次投递结果，不移动系统指针也不改变前台；WPF 等 toolkit 可以不支持它。此处不会使用 `AttachThreadInput`、权限提升或 UAC 绕过。
- Windows 当前提供 10 个 tool，其中前 9 个与 macOS 主线保持一致，并额外提供 `save_screenshot`：`list_apps`、`get_app_state`、`save_screenshot`、`click`、`perform_secondary_action`、`scroll`、`drag`、`type_text`、`press_key`、`set_value`。其中 element-targeted action 使用最新 snapshot 的 generation-bound opaque `element_index`，Go dispatcher 将完整 identifier 绑定到不可变 snapshot/runtime ID；成功 action 会发布新 snapshot，因此 action result 中的旧 identifier 立即失效；automation metadata 仅用于展示和模型选择，不能替代子元素身份，coordinate action 使用 screenshot/window-relative 坐标。
- `element_index` 是 generation-bound opaque snapshot identifier，不是持久的名称或数组位置。Windows 的元素动作只有在 dispatcher 能从最新 retained snapshot 取回完整 element record 时才进入 child resolver；该 record 必须带有非空、可比较的 `runtimeId`（每个组件必须是可表示的 signed 32-bit UIA 整数），并以已验证的 snapshot HWND 作为当前 UIA 树根进行 exact match；旧的裸数字 index、未知 token、过期 token、字符串空白、小数、null 成员和越界数值都 fail closed。`name`、`automationId`、`controlType` 只用于展示和选择，不能作为重定向身份。解析失败统一返回 `Target changed; call get_app_state again.`，并发生在 UIA pattern、frame 坐标计算、窗口消息或输入注入之前。每次成功 snapshot/action refresh 都发布新代次，旧 identifier 不得复用到新 record；语义操作仍可在没有 `frame` 时工作，coordinate-only click/drag 不携带 record、结构性地跳过子元素身份闸门。UIA provider 若在控件重建后重用 runtime ID，仍需要后续更强的 parent/path 或 provider-specific identity 设计；当前不会降低为 metadata fallback。
- Windows `click_method=accessibility` 映射到 UI Automation pattern；`auto` 仍保持 UIA 优先、后台 window-message fallback 的现有行为。`app_post` 始终保持消息定向：有原生 child HWND 的 WinForms button 使用 `BM_CLICK`，其他原生控件使用坐标消息；WPF/no-child-HWND 目标返回明确 capability error，绝不静默转为 global。坐标 fallback 统一要求有限、正尺寸的 snapshot `frame` 或明确的有限 window-relative `x/y`，缺少两者时在任何 point calculation 或 delivery 前 fail closed；semantic UIA click 仍可无 frame 执行。macOS-only 的 `sky_click` 仍会明确返回 unsupported。
...
- 可观察的 WPF 和 WinForms 验证夹具位于 `apps/OpenComputerUseWindows/fixtures/`；`run-interactive-smoke.ps1` 可由 PowerShell 7 或 Windows PowerShell 5.1 运行，默认用显式 `pwsh.exe` 启动 disposable WPF/WinForms 子进程，并用 `powershell.exe` 执行嵌入的 `runtime.ps1` 以实际覆盖 5.1 兼容性；`-FixtureHostPath` / `-RuntimeHostPath` 控制 WPF/runtime，`-NativeFixtureHostPath` 独立控制 WinForms host。由于 PS5.1/.NET Framework 的 WinForms UIA provider 可能把 child controls 全部暴露为 generic `Pane`，native-HWND positive 使用可暴露 `Edit`/`Button`/`Slider` 的 host，且 runner 输出记录各 host/version 边界；runner 在 PS7 使用 `ProcessStartInfo.ArgumentList`，在 PS5.1 使用带引号的 `Arguments` fallback。runner 覆盖 generation-bound identity、截图安全省略/PNG、numeric/identity malformed input、replacement/stale action、coordinate fallback、focus-only type_text、WPF capability 和 native HWND 路径；另有 opt-in Go identity/bounds smoke test。普通 fixture 脚本仍是交互式手工工具，runner 尚未覆盖完整第三方应用 action matrix。当前 Windows 侧仍没有 visual cursor overlay、installer/onboarding 或 code signing；后续 TODO 记录在 `docs/exec-plans/active/20260422-windows-computer-use-runtime.md`。
