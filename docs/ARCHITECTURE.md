...
- Windows runtime 默认只连接已经运行的 app，不会在 `get_app_state` 找不到进程时自动 `Start-Process`，也不会默认允许 `SetFocus` secondary action；这两条前台抢占路径分别需要 `OPEN_COMPUTER_USE_WINDOWS_ALLOW_APP_LAUNCH=1` 和 `OPEN_COMPUTER_USE_WINDOWS_ALLOW_FOCUS_ACTIONS=1` 显式打开。`get_app_state` 默认只返回 UI Automation 状态树；需要视觉像素时传入 `include_image: true`。显式请求图片时，Windows bridge 对当前拥有前台窗口的目标直接使用屏幕拷贝；后台目标先尝试 `PrintWindow`，检测黑图后回退到屏幕拷贝，并将 PNG Alpha 归一化为不透明；action tools 的刷新结果仍自动包含截图。`type_text` 只对动作时刻当前聚焦、且通过目标进程与 snapshot-bound window 归属校验的可写文本控件生效；调用方必须先 click/select 字段，不能依赖 `focusedSummary`、树顺序或同 metadata 控件，`set_value` + `element_index` 是精确元素赋值替代。运行时不会扫描当前 UIA 树寻找替代控件，也不会向顶层 HWND 发送兜底键盘消息；通过校验的目标优先使用 child HWND 的 `EM_SETSEL` / `EM_REPLACESEL`，没有可用 native handle 时才在 `OPEN_COMPUTER_USE_WINDOWS_ALLOW_UIA_TEXT_FALLBACK=1` 显式授权下使用同一 focused element 的 UIA `ValuePattern.SetValue`。
...
- Windows 当前提供 10 个 tool，其中前 9 个与 macOS 主线保持一致，并额外提供 `save_screenshot`：`list_apps`、`get_app_state`、`save_screenshot`、`click`、`perform_secondary_action`、`scroll`、`drag`、`type_text`、`press_key`、`set_value`。其中 element-targeted action 会优先复用上一轮 `get_app_state` 的 runtime ID；automation metadata 仅用于展示和模型选择，不能替代子元素身份，coordinate action 使用 screenshot/window-relative 坐标。
- `element_index` 仍然是 snapshot-local 身份，不是持久的名称或数组位置。Windows 的元素动作只有在操作实际携带 element record、且该记录带有非空、可比较的 `runtimeId`（每个组件必须是可表示的 signed 32-bit UIA 整数）时才进入 child resolver，并以已验证的 snapshot HWND 作为当前 UIA 树根进行 exact match；字符串、空白、小数、null 成员和越界数值都拒绝。`name`、`automationId`、`controlType` 只用于展示和选择，不能作为重定向身份。解析失败统一返回 `Target changed; call get_app_state again.`，并发生在 UIA pattern、frame 坐标计算、窗口消息或输入注入之前。语义操作仍可在没有 `frame` 时工作，coordinate-only click/drag 不携带 record、结构性地跳过子元素身份闸门。UIA provider 若在控件重建后重用 runtime ID，仍需要后续更强的 parent/path 或 provider-specific identity 设计；当前不会降低为 metadata fallback。
...
- 可观察的 WPF 和 WinForms 验证夹具位于 `apps/OpenComputerUseWindows/fixtures/`；`run-interactive-smoke.ps1` 提供 PowerShell 7 的 deterministic safety runner，另有 opt-in Go identity/bounds smoke test。普通 fixture 脚本仍是交互式手工工具，runner 尚未覆盖完整第三方应用 action matrix。当前 Windows 侧仍没有 visual cursor overlay、installer/onboarding 或 code signing；后续 TODO 记录在 `docs/exec-plans/active/20260422-windows-computer-use-runtime.md`。
