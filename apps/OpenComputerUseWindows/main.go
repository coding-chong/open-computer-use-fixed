	...
	return []toolDefinition{
		{
			...
			InputSchema: objectSchema(map[string]any{
				...
				"x":             coordinateProperty("X coordinate in physical pixels of the snapshot image; if the image you are shown is scaled, convert a fraction of the window frame instead of reusing rendered pixels"),
				"y":             coordinateProperty("Y coordinate in physical pixels of the snapshot image; if the image you are shown is scaled, convert a fraction of the window frame instead of reusing rendered pixels"),
				...
				"click_method":  enumStringProperty("Click implementation: auto (default), accessibility, app_post, sky_click, or global. Accessibility requires element_index. Windows supports app_post through HWND messages; global requires explicit foreground and global-pointer environment authorization. Windows does not support sky_click. On a Chromium/Electron content area the accessibility tree commonly exposes no actionable nodes, so x/y coordinates are the only path: app_post resolves the child window under the point and works even while the target is occluded or not foreground, whereas global sends real pointer input and is refused unless the target is the topmost window at that exact point. auto performs an element action when one exists and otherwise posts window messages to the top-level window (app_post instead targets the child window under the point), so an unchanged screenshot after a coordinate click usually means the point was wrong, not that the method cannot reach the app.", clickMethodValues),
			}, []string{"app"}),
		},
		{
			Name:        "drag",
			...
			InputSchema: objectSchema(map[string]any{
				"app":    stringProperty("App name or bundle identifier"),
				"from_x": coordinateProperty("Start X coordinate in physical pixels of the snapshot image"),
				"from_y": coordinateProperty("Start Y coordinate in physical pixels of the snapshot image"),
				"to_x":   coordinateProperty("End X coordinate in physical pixels of the snapshot image"),
				"to_y":   coordinateProperty("End Y coordinate in physical pixels of the snapshot image"),
		...
		{
			Name:        "press_key",
			Description: "Press a key or key-combination on the keyboard, including modifier and navigation keys. Windows interactive input requires explicit foreground-input authorization and a foreground target; enabling the separate focus-actions flag permits a bounded foreground attempt, but Windows' foreground lock still refuses it while another process owns the foreground, so no flag can manufacture foreground ownership. If the target is fully occluded, raise that window itself first (or ask the operator), then one authorized global click on it grants the foreground. On a Chromium/Electron content area this is the only way to enter text: click or select the field, then press keys one at a time.\n  - This supports xdotool's `key` syntax.\n  - Examples: \"a\", \"Return\", \"Tab\", \"super+c\", \"Up\", \"KP_0\" (for the numpad 0). This tool is part of plugin `Computer Use`.",
			Annotations: defaultAnnotations(),
			InputSchema: objectSchema(map[string]any{
				"app": stringProperty("App name or bundle identifier"),
		...
		{
			Name:        "set_value",
			Description: "Set the value of a settable accessibility element. Requires a settable element from the accessibility tree; a target that exposes no actionable nodes (Chromium/Electron content areas) has no identifier to use, and its text must be entered with press_key after focusing the field. This tool is part of plugin `Computer Use`.",
			Annotations: defaultAnnotations(),
			InputSchema: objectSchema(map[string]any{
				"app":           stringProperty("App name or bundle identifier"),
				"element_index": stringProperty("Generation-bound element identifier returned by the latest get_app_state"),
		...
		{
			Name:        "type_text",
			Description: "Type literal text into the current focused writable text control owned by the requested Windows app and snapshot window. Click/select the field first; this tool never chooses a substitute control or uses top-level keyboard fallback. Use set_value with the complete generation-bound identifier in element_index for exact element targeting. On a Chromium/Electron content area the native focus is the render widget rather than a text control and the tree has no identifiers, so this tool and set_value both fail closed even when the field is visibly focused; use press_key for those targets, one key per call. This tool is part of plugin `Computer Use`.",
			Annotations: defaultAnnotations(),
			InputSchema: objectSchema(map[string]any{
				"app":  stringProperty("App name or bundle identifier"),
