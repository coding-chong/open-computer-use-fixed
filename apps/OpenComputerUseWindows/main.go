...
func toolDefinitions() []toolDefinition {
	return []toolDefinition{
		{
			Name:        "click",
			Description: "Click an element by index or pixel coordinates from screenshot. Element-targeted coordinate fallback requires a valid snapshot frame or explicit finite x/y; semantic accessibility clicks may work without a frame. This tool is part of plugin `Computer Use`.",
		...
		{
			Name:        "type_text",
			Description: "Type literal text into the current focused writable text control owned by the requested Windows app and snapshot window. Click/select the field first; this tool never chooses a substitute control or uses top-level keyboard fallback. Use set_value with element_index for exact element targeting. This tool is part of plugin `Computer Use`.",
			Annotations: defaultAnnotations(),
			InputSchema: objectSchema(map[string]any{
				"app":  stringProperty("App name or bundle identifier"),
